<script>
    ...
    function handleSubmit() {
      ...
      setTimeout(() => {
        ...
        if (formData.carWashPackage !== 'none') {
          ...
          // These would typically come from the PriceCalculator component
          // Here we're simplifying with hardcoded calculations for demonstration
          const standardCarWashPrice = formData.carWashPackage === 'smartInteriorExterior' ? 8900 : 12000;
          const carWashDiscount = standardCarWashPrice * 0.20;
          const discountedCarWashPrice = standardCarWashPrice - carWashDiscount;
          
          // Get discounted daily rate based on stay length
          function getDiscountedDailyRate(days) {
            if (days <= 1) return 2400;
            if (days <= 3) return 2240;
            if (days <= 7) return 2000;
            if (days <= 14) return 1760;
            return 1600;
          }
          
          const parkingTotal = calculateDays() * getDiscountedDailyRate(calculateDays());
          ...
          bookingDetails.totalPrice = parkingTotal + discountedCarWashPrice;
        } else {
          // Only parking, no car wash
          function getDiscountedDailyRate(days) {
            if (days <= 1) return 2400;
            if (days <= 3) return 2240;
            if (days <= 7) return 2000;
          ...
          bookingDetails.totalPrice = parkingTotal;
        }
  ...
  <form class="booking-form" on:submit|preventDefault={handleSubmit}>
    <div class="form-section">
      ...
      <!-- Use the Price Calculator Component -->
      <PriceCalculator 
        formData={formData} 
