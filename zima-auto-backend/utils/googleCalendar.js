...
function getCalendarId(service) {
  ...
  // URL decode the calendar ID if it contains encoded characters
  const decodedCalendarId = decodeURIComponent(calendarId);
  console.log('Using calendar ID:', decodedCalendarId);
  return decodedCalendarId;
}
 ...
 */
async function checkTimeSlotAvailability(calendar, date, service, allPotentialSlots) {
  try {
    if (!allPotentialSlots || !Array.isArray(allPotentialSlots)) {
      console.error('Invalid or missing allPotentialSlots parameter');
      return [];
    }
