<script>
  ...
  // Service data - Only Tire Service and Auto Service for zima-service-frontend
  const services = [
    {
      id: 'tireService',
      color: '#b34700',
  ...
  // Handler for service card CTA buttons - navigate to booking page with preselected service
  function handleServiceAction(serviceId) {
    // Map service IDs to booking service IDs
    const bookingServiceId = serviceId === 'tireService' ? 'tireService' : 'autoService';
...
<section class="hero" bind:this={heroSection}>
  ...
  <div class="container hero-container">
    <div class="hero-content">
      <h1>{$currentLang === 'hu' ? 'Zima Autószerviz és Gumiszerviz' : 'Zima Car Maintenance and Tire Service'}</h1>
      <p>{$currentLang === 'hu'
      ...
      {#each services as service, i}
        <ServiceCard
          ...
          title={$currentLang === 'hu'
            ? (service.id === 'tireService' ? 'GUMISZERVIZ' : 'AUTÓSZERVIZ')
            : (service.id === 'tireService' ? 'TIRE SERVICE' : 'AUTO SERVICE')
          }
          description={$currentLang === 'hu'
            ? (service.id === 'tireService' ? 'Szakértő technikusaink készen állnak az abroncsokkal kapcsolatos bármilyen probléma azonnali megoldására, hogy biztonságosan visszatérhessen az útra.' :
               'A rutinellenőrzéstől a komplex javításokig szakképzett szerelőink fel vannak szerelve az összes karbantartás elvégzésére.')
            : (service.id === 'tireService' ? 'Our expert technicians are ready to solve any tire-related problems immediately and get you safely back on the road.' :
